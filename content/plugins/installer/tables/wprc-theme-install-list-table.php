	...
	function single_row( $theme, $actions = null, $show_details = true ) {
		...
		if (version_compare($wp_version,'3.4','>='))
		{
			...
			switch ( $status ) {
				default:
				case 'install':
					$actions[] = '<a class="install-now" href="' . esc_url( wp_nonce_url( $install_url, 'install-theme_' . $theme->slug ) ) . '" title="' . esc_attr( sprintf( __( 'Install %s' ,'installer'), $name ) ) . '">' . __( 'Install Now' ,'installer') . '</a>';
					break;
				case 'update_available':
					$actions[] = '<a class="install-now" href="' . esc_url( wp_nonce_url( $update_url, 'upgrade-theme_' . $theme->slug ) ) . '" title="' . esc_attr( sprintf( __( 'Update to version %s' ,'installer'), $theme->version ) ) . '">' . __( 'Update' ,'installer') . '</a>';
					break;
	...
	function install_theme_info( $theme ) {
		...
		if (($status=='newer_installed' || $status=='latest_installed') || !isset($theme->purchase_link) || empty($theme->purchase_link) /*&& (!isset($theme->message) || empty($theme->message))*/)
		{
			switch ( $status ) {
				default:
				case 'install':
					echo '<a class="theme-install button-primary" href="' . esc_url( wp_nonce_url( $install_url, 'install-theme_' . $theme->slug ) ) . '">' . __( 'Install' ,'installer') . '</a>';
					break;
				case 'update_available':
				...
				if ( is_object($update_themes) && isset($update_themes->response) ) {
					...
					echo '<a class="theme-install button-primary" href="' . esc_url( wp_nonce_url( $update_url, 'upgrade-theme_' . $theme->slug ) ) . '" title="' . esc_attr( sprintf( __( 'Update to version %s' ,'installer'), $theme->version ) ) . '">' . __( 'Update' ,'installer') . '</a>';
					break;
				case 'newer_installed':
				case 'latest_installed':
