...
if( ! function_exists( 'av_page_as_footer_message' ) )
{
	...
	function av_page_as_footer_message( array $params )
	{
		...
		if( ( 0 === $id ) || ( $footer_page != $id ) )
		{
			return $params;
		}
		
		$note = __( 'This page is currently selected to be displayed as footer. (Set in Enfold &raquo; Footer). Therefore it can not be accessed directly by the general public in your front end. (Logeed in users who are able to edit the page can still see it in the frontend)', 'avia_framework' );
            ...
            if ($maintenance_page) {
                ...
                if ( ($GLOBALS['pagenow'] !== 'wp-login.php') && !current_user_can('edit_published_posts')) {
                    ...
                    if ($maintenance_url !== $current_url) {
                        // do a simple redirect if WPML active
                        if( defined('ICL_SITEPRESS_VERSION') && defined('ICL_LANGUAGE_CODE')) {
