...
(function($)
{
	...
	var sidebar_colorpicker = function()
	{
		...
		$(document).on( 'widget-added', function( event, instance )
		{
			...
			{
				self.check_colorpickers( $( instance ) );
			}
			else
			{
	...
	sidebar_colorpicker.prototype =
	{
		check_colorpickers: function( widget )
		{
			...
			//	blocks are added dynamically by default - static in classic widget page
			var selector = ! $('body').hasClass( 'block-editor-page' ) ? '.avia-colorpicker-to-attach' : '';
			var colorpicker = widget_areas.find( '.avia_color_picker' + selector );
